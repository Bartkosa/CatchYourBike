...
from bikefinder.sources.njuskalo.client import fetch_search_pages as _fetch_njuskalo_raw
...
def _parse_posted_at_raw(
    ...
    *,
    ...
    """Slovenian-style chip → ISO 8601 with Ljubljana offset, or None."""
    if not raw:
        return None
    ...
    if m:
        ...
        dt = datetime(d.year, d.month, d.day, hh, mm, tzinfo=_LJUBLJANA)
        return dt.isoformat()

    m = re.match(
        r"Včeraj,\s*(\d{1,2}):(\d{2})\s*$",
        s,
        re.IGNORECASE,
    )
    if m:
    ...
    if m:
        ...
        if m.group(4):
            ...
            hh, minute = 12, 0
        dt = datetime(year, month, day, hh, minute, tzinfo=_LJUBLJANA)
        return dt.isoformat()

    return None


class NjuskaloSource:
    source_id = "njuskalo"
    ...
    def fetch_search_pages(
        ...
        *,
        ...
        listings, reached_end = _fetch_njuskalo_raw(
            cfg,
            search_url,
            search_name,
    ...
    def parse_posted_at(self, value: str | None) -> datetime | None:
        ...
        if s[0].isdigit() or s.startswith("20"):
            try:
                ...
                pass
        now = datetime.now(tz=_LJUBLJANA)
