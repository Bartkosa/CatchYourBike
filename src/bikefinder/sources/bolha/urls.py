...
_BOLHA_HOST = re.compile(r"(^|\.)bolha\.com$", re.IGNORECASE)


def assert_bolha_search_url(url: str) -> None:
    u = urlparse(url.strip())
    if u.scheme not in ("http", "https") or not _BOLHA_HOST.search(u.netloc or ""):
        raise ValueError(
            f"Bolha search url must be https://www.bolha.com/... got {url!r}"
        )
